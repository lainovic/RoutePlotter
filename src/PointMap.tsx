...
export default function PointMap({ points }: { points: NavigationPoint[] }) {
  const map = React.useRef<L.Map | null>(null);
  const pointMarkers = React.useRef<L.LayerGroup>(L.layerGroup());
  ...
  React.useLayoutEffect(() => {
    const newPointMapInstance = L.map("map");
    log("PointMap created");

    // Add tile layer from OpenStreetMap
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
      attribution: "&copy; OpenStreetMap contributors",
    }).addTo(newPointMapInstance);
    ...
    newPointMapInstance.on("click", (e) => {
        ...
        createPopup(
          ...
          "The distance between the points is: " +
            rulerStartMarker.current
              ...
              .toFixed(2) +
            " meters"
        ).openOn(newPointMapInstance);
      } else {
        cleanup(rulerStartMarker, rulerEndMarker, rulerLine);
      }
    });

    newPointMapInstance.on("contextmenu", async (e) => {
      const latlng = newPointMapInstance.mouseEventToLatLng(e.originalEvent);
    ...
    newPointMapInstance.setView([52.379189, 4.899431], 13);

    map.current = newPointMapInstance;

    return () => {
      newPointMapInstance.remove();
      log("PointMap removed");
    };
  }, []);
